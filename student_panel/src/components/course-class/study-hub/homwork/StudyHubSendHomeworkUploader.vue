...
const confirm = async ()=> {
  ...
  try {
    loading.value = true;
    await ClassRepository.homework(classId.value,{filepond: imageKeys});
...
watch(() => props.isElementarySchool,(value) => value ? addFileType(ELEMENTARY_SCHOOL_ADDITIONAL_FILE_TYPE) : '')
</script>

<template>
  <div class="mt-6">
    <FilePond
        :key="isElementarySchool"
        name="filepond[]"
        ref="pond"
