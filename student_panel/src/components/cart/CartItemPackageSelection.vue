...
const { product, sections, loading, initialSelectedItems } = useProductData(props.productId);
const { selectedCourses, select } = useSectionSelection(product,initialSelectedItems);
...
const expandRow = (item) => {
  if (!expanded.value.includes(item.id)) {
    ...
    model.value = { section_id: item.id,product_id: item.courses.find((course)=> course.is_selected).product_id}
  };
};

const collapseRow = (item) => {
  expanded.value = expanded.value.filter(expandedID=> expandedID !== item.id);
};
