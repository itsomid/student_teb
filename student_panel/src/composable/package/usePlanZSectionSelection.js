...
export function useSectionSelection(product,initialSelectedItems) {
    ...
    const sectionSelected = (sectionId) => {
        const teacherId = selectedCourses[sectionId];
        const section = product.value.sections.find(section => section.section_id == sectionId);
       if(section) return section.teachers.find(teacher => teacher.teacher_id === teacherId);
       else return false
    };
     ...
     */
    const select = (item) => {
        selectedCourses[item.section_id] = item.teacher_id;
    };
    ...
    watch(selectedCourses,(value)=>{
        console.log(value, '-value-',initialSelectedItems)
