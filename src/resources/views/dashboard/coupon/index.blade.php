            ...
            <div class="card-body border-bottom">
                ...
                <form class="row" action="{{route('admin.coupons.index')}}" method="get">
                    <div class="col-md-4 user_role">
                        ...
                        <input type="text" name="coupon_name" class="form-control" placeholder="کد تخفیف را وارد کنید"
                               value="{{request()->input('coupon_name')}}">
                        @error('coupon_name') <small class="text-danger">{{$message}}</small> @enderror
                    </div>
                    <div class="col-md-4">
                        ...
                        <select name="coupon_type"
                            ...
                            @foreach(\App\Models\Coupon::types()  as $key => $type)
                                ...
                                <option
                                    @selected(request()->has('coupon_type') && request()->input('coupon_type') == $key)  value="{{$key}}">{{$type}}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-12 mt-3">
                        ...
                        @can('coupons.excel')
                            <button type="submit" name="action" value="excel"
                                    class="btn btn-success mt-2 mx-2 text-white">
                                دانلود اکسل با اعمال فیلتر
                                <i class="fa-solid fa-file-excel mx-2"></i>
                            </button>
                        @endcan

                        @if(request()->hasAny(['coupon_name', 'creator_user_id', 'user_id', 'discount_percentage', 'description']))
                            ...
                            <div class="btn-group">
                                ...
                                <ul class="dropdown-menu" aria-labelledby="dropdownMenuButtonIcon">
                                    <li><a class="dropdown-item d-flex align-items-center"
                                           href="{{route('admin.coupons.create_specified_students_coupon')}}"><i
                                                class="ti ti-chevron-right scaleX-n1-rtl"></i> کد تخفیف معمولی</a></li>
                                    <li><a class="dropdown-item d-flex align-items-center"
                                           href="{{route('admin.coupons.create-mass-creation')}}"><i
                                                class="ti ti-chevron-right scaleX-n1-rtl"></i>کد تخفیف عمده</a></li>
                                    <li><a class="dropdown-item d-flex align-items-center"
                                           href="{{route('admin.coupons.create-conditional-student-discount')}}"><i
                                                class="ti ti-chevron-right scaleX-n1-rtl"></i>کد تخفیف شرطی</a></li>
                                </ul>
                            </div>
                        @endcan
                    </div>
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    @if($coupon->discount_percentage)
                                        ...
                                        %
                                    @else
                                        {{formatNumberWithSlashes($coupon->discount_amount)}}
                                        ریال
                                    @endif
                                </td>
                                <td class="text-center">
                                    {{\Morilog\Jalali\Jalalian::forge($coupon->expired_at)->format('%Y/%m/%d H:i')}}
                                </td>
                                <td class="text-center">
                                    N/A
                                </td>
                                <td class="d-flex align-items-center">
                                    <div class="dropdown mx-3">
                                        ...
                                        <div class="dropdown-menu">
                                            ...
                                            @can('coupons.destroy')
                                                <form
                                                    action="{{route('admin.coupons.destroy', ['coupon' => $coupon->id])}}"
                                                    method="post">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button class="dropdown-item" type="button" data-bs-toggle="modal"
                                                            data-bs-target="#exampleModal_{{$coupon->id}}"
                                        ...
                                        <div class="modal fade" id="exampleModal_{{$coupon->id}}" tabindex="-1"
                                             aria-labelledby="exampleModalLabel" aria-hidden="true">
                                            <div class="modal-dialog modal-dialog-top">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        <h5 class="modal-title" id="exampleModalLabel">تایید حذف کد
                                                            تخفیف</h5>
