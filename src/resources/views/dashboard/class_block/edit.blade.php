    ...
    <section class="form-control-repeater">
        <div class="card">
            ...
            <div class="card-body">
                ...
                <form class="row" method="post" action="{{route('admin.class-block.update', $classBlock->id)}}">
                    @csrf
                    @method('patch')
                    ...
                    <div class="col-md-6">
                        ...
                        <class-selection-component input-name="product_id" default-value='@json(['product_id' => $classBlock->product_id, 'product_name' => $classBlock->product->name])'></class-selection-component>
                    </div>
                    <div class="col-md-6">
                        <label for="selectStudent" class="form-label mb-0">کاربر:
                        </label>
                        <x-student-selection-component default-value="{{ $classBlock->student_id }}" input-name="user_id"></x-student-selection-component>
                    </div>
                    <div class="col-md-12">
                        <label for="numeral-mask" class="form-label mb-0">توضیحات بلاک (اختیاری):</label>
                        <textarea name="description" cols="50" rows="10" class="form-control">{{ $classBlock->description }}</textarea>
                        @error('description')<small class="text-danger">{{$message}}</small>@enderror
                    </div>

                    <div class="col-md-6 user_role mt-3">
                        <label class="form-label" for="user_description">تا چه زمانی بلاک باشد؟:</label>
                        <input class="form-control" data-jdp id="expired_at" name="expired_at" placeholder="توضیحات..." value="{{ $classBlock->expired_at() }}">
