...
@section('content')
                        ...
                        <div class="content-left">
                            ...
                            <div class="d-flex align-items-center my-1">
                                <h4 class="mb-0 me-2">{{ $verified_students_count }}
                                    @if ($students->total())
                                        (%{{ round(($verified_students_count / $students->total()) * 100) }})
                                </h4>
                                @endif
                            </div>
                        </div>
        ...
        <div class="card-body">
            ...
            <form class="row mt-3 d-flex align-items-end justify-content-between">
                <div class="col-md-4 user_role">
                    ...
                    <input type="number" class="form-control" placeholder="آیدی دانشجو">
                </div>
                <div class="col-md-4 user_role">
    ...
    <div class="card">
        <div class="card-body border-bottom">
            <h5 class="card-title">فیلتر دانشجوان</h5>
            <form class="row" action="{{ route('admin.student.index') }}" method="get">
                ...
                <div class="col-md-4 user_status">
                    ...
                    <select id="referral_code_id" name="referral_code_id" class="form-select text-capitalize mb-md-0 ">
                        <option value=""> همه</option>

                        @foreach ($referral_codes as $referral_code)
                            <option value="{{ $referral_code->id }}" class="text-capitalize"
                                {{ request()->input('referral_code_id') == $referral_code->id ? 'selected' : null }}>
                                {{ $referral_code->code }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-4 user_status mt-2">
                    ...
                    <select id="gender" class="form-select text-capitalize mb-md-0 " name="gender">
                        <option value=""> همه</option>
                        <option value="male" class="text-capitalize"
                            {{ request()->input('gender') == 'male' ? 'selected' : null }}>
                            پسر
                        </option>
                        <option value="female" class="text-capitalize"
                            {{ request()->input('gender') == 'female' ? 'selected' : null }}>
                            دختر
                        </option>
                    </select>
                </div>
                <div class="col-md-4 user_status mt-2">
                    ...
                    <select id="field_of_study" class="form-select text-capitalize mb-md-0 " name="field_of_study">
                        <option value=""> همه</option>

                        @foreach (\App\Data\FieldOfStudy::get() as $key => $field_of_study)
                            <option value="{{ $key }}"
                                {{ request()->input('field_of_study') === $key ? 'selected' : null }}
                                class="text-capitalize">{{ $field_of_study }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-4 user_status mt-2">
                    ...
                    <select id="grade" name="grade" class="form-select text-capitalize mb-md-0 ">
                        ...
                        @foreach (\App\Data\Grades::get() as $key => $grade)
                            <option value="{{ $key }}"
                                {{ request()->input('grade') == $key ? 'selected' : null }} class="text-capitalize">
                                {{ $grade }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="col-md-4 user_status mt-2">
                    ...
                    <select id="sales_description" name="sales_description" class="form-select text-capitalize mb-md-0 ">
                        <option value="">همه</option>
                        @foreach ($salesDescriptions as $key => $desc)
                            <option value="{{ $desc['sales_description'] }}"
                                {{ request()->input('sales_description') === $desc['sales_description'] ? 'selected' : null }}>
                                {{ $desc['sales_description'] }}</option>
                        @endforeach
                    </select>
                </div>
        ...
        <div class="card-body">
            <div class="card-title header-elements">
                <h5 class="m-0 me-2">لیست دانشجوان</h5>
                        ...
                        <tbody class="table-border-bottom-0">
                            ...
                            @foreach ($students as $student)
                                <tr>
                                    ...
                                    <td>
                                        @if ($student->referrer)
                                            {{ $student->referrer?->code }}
                                            ({{ $student->referrer?->admin?->fullname() }})
                                        @endif
                                    ...
                                    <td class="d-flex align-items-center">
                                        ...
                                        <div class="dropdown mx-3">
                                            ...
                                            <div class="dropdown-menu">
                                                ...
                                                <a class="dropdown-item"
                                                    href="{{ route('admin.student.change-password', ['student' => $student->id]) }}">
                                        ...
                                        @can('student.edit-note')
                                            <note-modal :id="'noteModal' + {{ $student->id }}"
                                                :url="'{{ route('api.student.update-note', ['student' => $student->id]) }}'"
                                                :sales_description="'{{ $student->sales_description }}'"></note-modal>
                                        @endcan
                                    </td>
                                </tr>
                            @endforeach
            ...
            @else
                <p class="text-center h4 mt-5">شما هنوز دانشجوی نداری🙄</p>
            @endif
        </div>
